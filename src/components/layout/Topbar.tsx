...
export function Topbar({ user }: TopbarProps) {
  ...
  // สร้าง Breadcrumb / Title ภาษาไทย
  const getPageTitle = () => {
    ...
    if (pathname.includes("/schedules") || pathname.includes("/schedule") || pathname.includes("/timetable")) return "ตารางเรียนและตารางสอน";
  ...
  return (
    <header className="sticky top-0 z-40 flex items-center justify-between h-16 px-4 md:px-8 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shrink-0">
      ...
      {/* ฝั่งขวา: Academic Year Badge + ThemeToggle + User Mini Info */}
      <div className="flex items-center gap-2 md:gap-4">
        <Badge variant="outline" className="hidden sm:inline-flex text-xs font-normal border-primary/30 text-primary bg-primary/5">
          ภาคเรียนที่ 1/2569
        </Badge>
        ...
        <div className="hidden lg:flex items-center gap-2 pl-2 border-l border-border">
          ...
          <div className="text-left leading-tight">
            <p className="text-xs font-semibold text-foreground truncate max-w-[130px]">{user.fullName}</p>
